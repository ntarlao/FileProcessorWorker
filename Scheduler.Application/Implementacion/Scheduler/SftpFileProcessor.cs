...
namespace Scheduler.Application.Implementacion.Scheduler
{
    ...
    public class SftpFileProcessor : ISftpFileProcessor
    {
        ...
        public SftpFileProcessor(IArchivosService archivosService, IConfiguration configuration, IErrorNotifier errorNotifier, Func<string, Stream>? openRemoteStream = null)
        {
        ...
        // Stream que encapsula el stream real y libera también el propietario (SftpClient). Lo uso para testing
        ...
        public async Task<List<RegistroTransaccion>> LeerArchivoAsync(string remoteFilePath, string? connectionId = null, CancellationToken cancellationToken = default)
        {
            ...
            for (int i = 0; i < degreeOfParallelism; i++)
            {
                consumers.Add(Task.Run(async () =>
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        //_ = _errorNotifier.NotifyErrorAsync("Error procesando líneas del archivo", new { remoteFilePath, exception = ex.ToString() }, connectionId, CancellationToken.None);
            ...
            try
            {
                ...
                while (!ctsToken.IsCancellationRequested)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        //await _errorNotifier.NotifyErrorAsync("Error escribiendo línea en canal", new { remoteFilePath, exception = ex.ToString() }, connectionId, CancellationToken.None).ConfigureAwait(false);
            ...
            catch (Exception ex)
            {
                ...
                //await _errorNotifier.NotifyErrorAsync("Error abriendo/leyendo stream remoto", new { remoteFilePath, exception = ex.ToString() }, connectionId, CancellationToken.None).ConfigureAwait(false);
